...
import { openTicketNoteView } from '../../views/TicketNoteView';
...
export class TicketModal extends Modal {
	...
	private endDate: number | undefined = undefined;
	...
	constructor(app: App, plugin: ProjectFlowPlugin, context: TicketModalContext, onSave: () => void) {
		...
		if (isEditContext(context)) {
			...
			this.endDate = context.ticket.endDate;
			...
			if (context.endDate) this.endDate = context.endDate;
	...
	onOpen(): void {
		...
		contentEl.addEventListener('keydown', (e: KeyboardEvent) => {
			...
			if (e.key === 'Delete' && (e.ctrlKey || e.metaKey) && isEdit) {
				...
				}, 'Delete', this.plugin.store.getDescendantIds(ticketId).map(id => this.plugin.store.getTicket(id)?.title ?? '').filter(Boolean)).open();
			}
		});
		...
		if (isEdit) {
			...
			openBtn.addEventListener('click', () => {
				this.close();
				openTicketNoteView(this.plugin, ticket.id);
			});
		}
		...
		// ── Parent ticket (edit mode only, subtask type) ──────────────────────
		if (isEditContext(this.context)) {
			const ticket = (this.context as EditTicketContext).ticket;
			if (ticket.type === 'subtask' && ticket.parentId) {
				const parent = this.plugin.store.getTicket(ticket.parentId);
				if (parent) {
					const parentBlock = formPane.createDiv('pf-desc-block');
					parentBlock.createEl('span', { cls: 'pf-desc-label', text: 'Parent ticket' });
					const parentRow = parentBlock.createDiv('pf-modal-subtask-row');
					const statuses = this.plugin.store.getProjectStatuses(parent.projectId);
					const statusDef = statuses.find(s => s.id === parent.status);
					parentRow.createEl('span', { cls: 'pf-badge pf-subtask-status-badge', text: statusDef?.label ?? parent.status });
					const parentTitle = parentRow.createEl('span', { cls: 'pf-modal-subtask-title', text: parent.title });
					parentTitle.addEventListener('click', () => {
						new TicketModal(this.app, this.plugin, { ticket: parent, sprintId: parent.sprintId ?? null }, () => {
							this.onSave();
						}).open();
					});
				}
		...
		// ── Subtasks / child tickets (edit mode only, non-subtask types) ──────
		if (isEditContext(this.context)) {
			const ticket = (this.context as EditTicketContext).ticket;
			if (ticket.type !== 'subtask') {
				const isEpic = ticket.type === 'epic';
				const sectionLabel = isEpic ? 'Child tickets' : 'Subtasks';
				const addLabel = isEpic ? '+ Child' : '+ Subtask';
				const defaultChildType: TicketType = isEpic ? 'task' : 'subtask';

				const subtasksBlock = formPane.createDiv('pf-desc-block');
				const headRow = subtasksBlock.createDiv('pf-modal-subtasks-head');
				headRow.createEl('span', { cls: 'pf-desc-label', text: sectionLabel });
				const addBtn = headRow.createEl('button', { cls: 'pf-btn pf-btn-sm', text: addLabel });

				const listEl = subtasksBlock.createDiv('pf-modal-subtasks-list');

				const renderSubtasks = () => {
					listEl.empty();
					const children = this.plugin.store.getChildTickets(ticket.id);
					if (children.length === 0) {
						listEl.createEl('div', { cls: 'pf-modal-subtasks-empty', text: 'None yet' });
					} else {
						const statuses = this.plugin.store.getProjectStatuses(ticket.projectId);
						for (const child of children) {
							const row = listEl.createDiv('pf-modal-subtask-row');
							const statusDef = statuses.find(s => s.id === child.status);
							row.createEl('span', { cls: 'pf-badge pf-subtask-status-badge', text: statusDef?.label ?? child.status });
							const titleEl = row.createEl('span', { cls: 'pf-modal-subtask-title', text: child.title });
							titleEl.addEventListener('click', () => {
								new TicketModal(this.app, this.plugin, { ticket: child, sprintId: child.sprintId ?? null }, () => {
									renderSubtasks();
									this.onSave();
								}).open();
							});
				...
				addBtn.addEventListener('click', () => {
					new TicketModal(this.app, this.plugin, {
		...
		if (this.endDate !== undefined) {
			const dd = new Date(this.endDate);
			...
			if (isArchived) {
				...
				footer.createEl('button', { cls: 'pf-btn', text: 'Archive' })
					.addEventListener('click', () => {
						...
						}, 'Archive', this.plugin.store.getDescendantIds(ticket.id).map(id => this.plugin.store.getTicket(id)?.title ?? '').filter(Boolean)).open();
					});
			}
