...
import { safeFileName, generateTicketNote } from '../../notes/ticketNote';
	...
	private async submit(): Promise<void> {
		...
		if (this.project) {
			...
			if (switchingToNoSprint) {
				...
				// Regenerate notes for all non-archived board tickets (non-backlog status)
				const backlogStatusIds = new Set(
					this.plugin.store.getProjectStatuses(this.project.id)
						.filter(s => s.universalId === "backlog")
						.map(s => s.id)
				);
				const boardTickets = this.plugin.store.getTickets({ projectId: this.project.id })
					.filter(t => !t.archived && !backlogStatusIds.has(t.status));
