...
export class NoteSyncWatcher {
	...
	private async syncNoteToStore(file: TFile): Promise<void> {
		...
		const validStatuses  = new Set(plugin.store.getProjectStatuses(ticket.projectId).map(s => s.id));
		const validPriorities = new Set<string>(['none', 'low', 'medium', 'high', 'critical']);
		const newStatus: TicketStatus     = validStatuses.has(fm.status)     ? fm.status     : ticket.status;
		const newPriority: TicketPriority = validPriorities.has(fm.priority) ? fm.priority as TicketPriority : ticket.priority;
		const rawPoints = fm.points;
		const newPoints: number | undefined = typeof rawPoints === 'number' && rawPoints >= 0
			? Math.round(rawPoints)
			: ticket.points;

		// Parse end date/time — new format: end_date + end_time; fall back to legacy end:
		let newEndDate: number | undefined = ticket.endDate;
		if (fm.end_date !== undefined) {
			// Field present (new format) — empty string or null means "clear"
			if (typeof fm.end_date === 'string' && fm.end_date.trim()) {
				const timeStr = typeof fm.end_time === 'string' && fm.end_time.trim() ? fm.end_time.trim() : '00:00';
				const parsed = new Date(`${fm.end_date.trim()}T${timeStr}`).getTime();
				if (!isNaN(parsed)) newEndDate = parsed;
			} else {
				newEndDate = undefined;
			}
		} else {
			// Legacy: combined end: field
			if (typeof fm.end === 'string' && fm.end.trim()) {
				const parsed = new Date(fm.end.trim()).getTime();
				if (!isNaN(parsed)) newEndDate = parsed;
			} else if (fm.end === null) {
				newEndDate = undefined;
			}
		...
		// Parse start date/time — new format: start_date + start_time; fall back to legacy start:
		let newStartDate: number | undefined = ticket.startDate;
		if (fm.start_date !== undefined) {
			// Field present (new format) — empty string or null means "clear"
			if (typeof fm.start_date === 'string' && fm.start_date.trim()) {
				const timeStr = typeof fm.start_time === 'string' && fm.start_time.trim() ? fm.start_time.trim() : '00:00';
				const parsed = new Date(`${fm.start_date.trim()}T${timeStr}`).getTime();
				if (!isNaN(parsed)) newStartDate = parsed;
			} else {
				newStartDate = undefined;
			}
		} else {
			// Legacy: combined start: field
			if (typeof fm.start === 'string' && fm.start.trim()) {
				const parsed = new Date(fm.start.trim()).getTime();
				if (!isNaN(parsed)) newStartDate = parsed;
			} else if (fm.start === null) {
				newStartDate = undefined;
			}
		}
		...
		if (typeof fm.recurrence === 'string' && validRecurRules.has(fm.recurrence)) {
			...
			if (rule === 'custom' && Array.isArray(fm.recurrence_custom_days)) {
				customDays = fm.recurrence_custom_days
					.filter((d: unknown) => typeof d === 'number' && d >= 0 && d <= 6)
					.map((d: number) => d);
			}
			newRecurrence = { rule, interval, endDate, customDays };
