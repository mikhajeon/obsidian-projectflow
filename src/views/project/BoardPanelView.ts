			...
			menu.addItem(item =>
				item.setTitle('Archive').setIcon('archive').onClick(() => {
					new ConfirmModal(this.view.app, `Archive "${ticket.title}"? It will be hidden from active views and can be restored later.`, async () => {
						...
						this.view.render();
					}, 'Archive', this.view.plugin.store.getDescendantIds(ticket.id).map(id => this.view.plugin.store.getTicket(id)?.title ?? '').filter(Boolean)).open();
				})
			);
			menu.addItem(item =>
				item.setTitle('Delete ticket').setIcon('trash').onClick(() => {
					new ConfirmModal(this.view.app, `Delete "${ticket.title}"? This cannot be undone.`, async () => {
						...
						this.view.render();
					}, 'Delete', this.view.plugin.store.getDescendantIds(ticket.id).map(id => this.view.plugin.store.getTicket(id)?.title ?? '').filter(Boolean)).open();
				})
			);
