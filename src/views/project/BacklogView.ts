...
export class BacklogView extends ItemView {
		...
		row.addEventListener('contextmenu', (e) => {
			...
			menu.addItem(item =>
				item.setTitle('Delete').setIcon('trash').onClick(() => {
					...
					}, 'Delete', this.plugin.store.getDescendantIds(ticket.id).map(id => this.plugin.store.getTicket(id)?.title ?? '').filter(Boolean)).open();
				})
			);
	...
	private openTicketNote(ticket: Ticket): void {
