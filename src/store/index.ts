...
import { migrateAppData, defaultTagFromName } from './migrate';
...
export class ProjectStore {
	 ...
	 * Sprint tickets (sprintId != null) are unassigned from their sprint so they
	 * appear in board and backlog views based on their status.
	...
	async createTicket(data: Omit<Ticket, 'id' | 'createdAt' | 'updatedAt' | 'order' | 'backlogOrder' | 'ticketNumber'>): Promise<Ticket> {
		// Subtasks must be attached to a task, bug, or story
		if (data.type === 'subtask') {
			const parent = data.parentId ? this.getTicket(data.parentId) : null;
			if (!parent || !['task', 'bug', 'story'].includes(parent.type)) {
				throw new Error('Subtasks must be attached to a task, bug, or story.');
			}
		}
		this.snapshotTickets();
